...
*/
public class CassandraWriteUnstructured100 extends AbstractBenchmark {

    private CqlSession session;
    private String img_blob;

    public CassandraWriteUnstructured100() {
        category = "cassandra";
    ...
    public void runQuery() {
        for (int i=0; i<100; i++) {
            session.execute(
                String.format(
                    "UPDATE tbl SET col2 = col2 + {'cat_%d': textAsBlob('%s')} WHERE col1=1", i, img_blob
                )
            );
        }
    }
